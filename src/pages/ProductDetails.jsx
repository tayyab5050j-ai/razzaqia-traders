...
export default function ProductDetails() {
  ...
  const submitReview = async () => {
    if (!name.trim() || !reviewText.trim()) { alert("Please fill all fields"); return; }
    await addDoc(collection(db, "reviews"), {
      productId: id, name, review: reviewText, rating, createdAt: Date.now(),
    });
    setName(""); setReviewText(""); setRating(5);
    loadReviews();
  };

  const loadRelatedProducts = async (currentCategory) => {
    const snapshot = await getDocs(collection(db, "products"));
    const data = snapshot.docs
      .map((doc) => ({ id: doc.id, ...doc.data() }))
      .filter((item) => item.id !== id && item.category === currentCategory)
      .slice(0, 4);
      ...
      <div className="product-details">
        <div className="product-details-image">
          ...
          <div className="product-thumbnails">
            <img src={product.imageUrl} alt="" onClick={() => setSelectedImage(product.imageUrl)} />
            {product.imageUrl2 && (
              <img src={product.imageUrl2} alt="" onClick={() => setSelectedImage(product.imageUrl2)} />
            )}
          </div>
        </div>
        ...
        <div className="product-details-info">
          ...
          <p>Premium electronics product available at Razzaqia Traders.</p>
      ...
      <div className="product-bottom-layout">
        <div className="related-products-sidebar">
          ...
          {relatedProducts.map((item) => (
            <Link key={item.id} to={`/product/${item.id}`} className="related-mini-card">
              <img src={item.imageUrl} alt={item.name} className="related-product-image" />
              <h4>{item.name}</h4>
          ...
          <div className="reviews-section">
            ...
            <div className="review-form">
              ...
              <div className="rating-selector">
                {[1,2,3,4,5].map((star) => (
                  <span key={star} className={star <= rating ? "star active-star" : "star"} onClick={() => setRating(star)}>★</span>
                ))}
              </div>
